...
void SegmentSol::envoyerMesure(string type) {
    ...
    else if (type.find(FrameManager::IMAGE) != string::npos) {
//            char monTableau[JPEG_BUF_SIZE];
//        (CameraPhoto) (leSegment->getInstrument())->getPhoto(monTableau); 
        int tailleIMG;
        int tailleDernTrame;
        int numTrame;
        int indiceEnCours;
        list<Mesure*> mesures = leSegment->getInstrument()->getMesures();
        message->setMesures(mesures);       
        list<char> laPhoto =mesures.back()->getDataListC();
        tailleIMG = laPhoto.size();

        tailleDernTrame = tailleIMG - (tailleIMG / 98)*98; // /nous donne une valeur entiere 
       
        tramerHeader(Message* message, tailleIMG, tailleDernTrame);
        Ret = LS.Open(DEVICE_PORT, 9600);
        Ret = LS.Write(trameEmission, trameEmission[1] + 5);
        LS.Close();        
        indiceEnCours = 0;
        do {
        tramerIMAGE(Message* message, numTrame, indiceEnCours);
        Ret = LS.Open(DEVICE_PORT, 9600);
        Ret = LS.Write(trameEmission, 100);
        LS.Close();     
        }
        while ();
